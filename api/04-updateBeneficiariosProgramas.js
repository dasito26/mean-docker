    ...
    for (let beneficiario of beneficiarios) {
      ...
      if (programa) {
        ...
        await Beneficiario.updateOne(
          ...
          { $set: { programa: programa._id } } // Añadir o actualizar el campo `municipio_id`
        );
        console.log(
          `${num} Actualizado beneficiario ${beneficiario._id} con programa_id ${programa._id}`
        );
      } else {
        console.log(
