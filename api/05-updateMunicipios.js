    ...
    for (let distrito of distritos) {
      ...
      if (municipio) {
        ...
        await Municipio.updateOne(
          { _id: municipio._id }, // Condición para actualizar el documento correcto
          { $set: { distrito: distrito.distrito } } // Añadir o actualizar el campo `municipio_id`
        );
        console.log(
          `Actualizado municipio ${municipio.nombre} con distrito ${distrito.distrito}`
        );
      } else {
        console.log(
          `No se encontró municipio para cve_mpio: ${municipio.cve_mpio_full}`
        );
      }
    }
